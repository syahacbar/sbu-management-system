        ...
        @else
            ...
            <section class="rounded-lg border border-slate-200 bg-white">
                <div class="flex flex-col gap-4 border-b border-slate-200 p-5 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <h3 class="text-lg font-semibold text-slate-950">Daftar Tenaga Ahli</h3>
                        <p class="mt-1 text-sm text-slate-500">Data PJTBU, PJSKBU, dan Tenaga Ahli untuk pengajuan SBU aktif.</p>
                    </div>

                    <a href="{{ route('companies.workspace.experts.create', $company) }}" class="inline-flex items-center justify-center rounded-md bg-emerald-700 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-800">
                        Tambah Tenaga Ahli
                    </a>
                </div>
                ...
                <div class="border-b border-slate-200 p-5">
                    <form method="GET" action="{{ route('companies.workspace.experts.index', $company) }}" class="flex flex-wrap items-center gap-3">
                        <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">Filter Tipe:</span>
                        
                        <a href="{{ route('companies.workspace.experts.index', [$company, 'type' => '']) }}" 
                           class="rounded-md border px-3 py-1.5 text-xs font-semibold transition {{ $type === '' ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50' }}">
                            Semua Tipe
                        </a>

                        <a href="{{ route('companies.workspace.experts.index', [$company, 'type' => 'pjtbu']) }}" 
                           class="rounded-md border px-3 py-1.5 text-xs font-semibold transition {{ $type === 'pjtbu' ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50' }}">
                            PJTBU
                        </a>

                        <a href="{{ route('companies.workspace.experts.index', [$company, 'type' => 'pjskbu']) }}" 
                           class="rounded-md border px-3 py-1.5 text-xs font-semibold transition {{ $type === 'pjskbu' ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50' }}">
                            PJSKBU
                        </a>

                        <a href="{{ route('companies.workspace.experts.index', [$company, 'type' => 'tenaga_ahli']) }}" 
                           class="rounded-md border px-3 py-1.5 text-xs font-semibold transition {{ $type === 'tenaga_ahli' ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50' }}">
                            Tenaga Ahli
                        </a>
                    </form>
                </div>
                            ...
                            @forelse ($experts as $item)
                                <tr>
                                    <td class="px-5 py-4">
                                        ...
                                        <p class="mt-0.5 text-xs text-slate-500 font-mono">NIK: {{ $item->nik }}</p>
                                    </td>
                                    <td class="px-5 py-4">
                                        @php
                                            $typeBadge = match($item->expert_type) {
                                                'pjtbu' => 'bg-indigo-50 border-indigo-200 text-indigo-700',
                                                'pjskbu' => 'bg-purple-50 border-purple-200 text-purple-700',
                                    ...
                                    <td class="px-5 py-4 font-mono text-slate-700 text-xs">
                                        {{ $item->npwp ?: '-' }}
                                    </td>
                                    <td class="px-5 py-4 space-y-0.5">
                                        <p class="font-bold text-slate-900">{{ $item->skk_registration_number ?: '-' }}</p>
                                        <div class="flex items-center gap-1.5 text-xs text-slate-600">
                                            <span class="font-semibold text-slate-400">Klas/Sub:</span>
                                            <span>{{ $item->skk_classification ?: '-' }} &rarr; {{ $item->skk_subclassification ?: '-' }}</span>
                                        </div>
                                        <div class="flex items-center gap-1.5 text-xs text-slate-600">
                                            <span class="font-semibold text-slate-400">Kualifikasi:</span>
                                            <span>{{ $item->skk_qualification ?: '-' }} (Jenjang {{ $item->skk_level ?: '-' }})</span>
                                        </div>
                                    </td>
                                    <td class="px-5 py-4 text-xs text-slate-700">
                                        <p><span class="text-slate-400">Terbit:</span> {{ $item->skk_issued_at?->format('d/m/Y') ?: '-' }}</p>
                                        <p class="mt-0.5"><span class="text-slate-400">Expired:</span> {{ $item->skk_expired_at?->format('d/m/Y') ?: '-' }}</p>
                                    </td>
                                    <td class="px-5 py-4">
                                        <div class="flex justify-end gap-1.5">
                                            <a href="{{ route('companies.workspace.experts.edit', [$company, $item]) }}" class="rounded-md border border-slate-300 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-50">
                                                Edit
                                            </a>

        ...
        @else
            ...
            <section class="rounded-lg border border-slate-200 bg-white">
                <div class="flex flex-col gap-4 border-b border-slate-200 p-5 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <h3 class="text-lg font-semibold text-slate-950">Daftar Peralatan</h3>
                        <p class="mt-1 text-sm text-slate-500">Daftar inventaris peralatan utama untuk syarat klasifikasi pengajuan.</p>
                    </div>

                    <a href="{{ route('companies.workspace.equipment.create', $company) }}" class="inline-flex items-center justify-center rounded-md bg-emerald-700 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-800">
                        Tambah Peralatan
                    </a>
                </div>
                ...
                <div class="border-b border-slate-200 p-5">
                    <form method="GET" action="{{ route('companies.workspace.equipment.index', $company) }}" class="flex flex-wrap items-center gap-3">
                        <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">Kategori Peralatan SBU:</span>
                        
                        <a href="{{ route('companies.workspace.equipment.index', [$company, 'category' => '']) }}" 
                           class="rounded-md border px-3 py-1.5 text-xs font-semibold transition {{ $category === '' ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50' }}">
                            Semua Peralatan
                        </a>

                        <a href="{{ route('companies.workspace.equipment.index', [$company, 'category' => 'bg']) }}" 
                           class="rounded-md border px-3 py-1.5 text-xs font-semibold transition {{ $category === 'bg' ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50' }}">
                            BG (Bangunan Gedung)
                        </a>

                        <a href="{{ route('companies.workspace.equipment.index', [$company, 'category' => 'bs']) }}" 
                           class="rounded-md border px-3 py-1.5 text-xs font-semibold transition {{ $category === 'bs' ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50' }}">
                            BS (Bangunan Sipil)
                        </a>
                    </form>
                </div>
                                ...
                                <tr>
                                    <td class="px-5 py-4">
                                        ...
                                        @if($item->masterEquipment)
                                            <p class="mt-0.5 text-xs text-slate-500 font-mono">Kode Master: {{ $item->masterEquipment->code }}</p>
                                        @endif
                                    </td>
                                    <td class="px-5 py-4">
                                        @php
                                    ...
                                    <td class="px-5 py-4">
                                        @php
                                            $ownBadge = match($item->ownership_status) {
                                                'milik_sendiri' => 'bg-emerald-50 text-emerald-700 border-emerald-200',
                                                'sewa' => 'bg-indigo-50 text-indigo-700 border-indigo-200',
                                    ...
                                    <td class="px-5 py-4">
                                        <div class="flex justify-end gap-1.5">
                                            <a href="{{ route('companies.workspace.equipment.edit', [$company, $item]) }}" class="rounded-md border border-slate-300 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-50">
                                                Edit
                                            </a>

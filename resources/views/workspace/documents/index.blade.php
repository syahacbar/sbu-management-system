        ...
        @else
            ...
            <section class="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
                <div class="flex flex-col gap-4 border-b border-slate-200 dark:border-slate-700 p-5 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <h3 class="text-lg font-semibold text-slate-950 dark:text-white">Daftar Dokumen Pendukung</h3>
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Daftar dokumen persyaratan administrasi perusahaan yang sah dan terverifikasi.</p>
                    </div>

                    <a href="{{ route('companies.workspace.documents.create', $company) }}" class="inline-flex items-center justify-center rounded-md bg-emerald-700 dark:bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-800 dark:hover:bg-emerald-700">
                        Unggah Dokumen Baru
                    </a>
                </div>
                        ...
                        <tbody class="divide-y divide-slate-200 dark:divide-slate-700 bg-white dark:bg-slate-800">
                            @forelse ($documents as $item)
                                <tr>
                                    ...
                                    <td class="px-5 py-4">
                                        @if ($item->file_path)
                                            <p class="text-xs text-slate-700 dark:text-slate-300 font-mono truncate max-w-[200px]" title="{{ $item->original_filename }}">
                                                {{ $item->original_filename }}
                                            </p>
                                        @else
                                            <span class="text-xs text-slate-400 dark:text-slate-500 font-medium italic">Tidak ada berkas</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-5 py-4 text-slate-600 dark:text-slate-400 text-xs">
                                        @if ($item->expired_at)
                                            <span class="{{ $item->expired_at->isPast() ? 'text-red-600 dark:text-red-400 font-bold' : '' }}">
                                                {{ $item->expired_at->format('d/m/Y') }}
                                                @if ($item->expired_at->isPast())
                                                    <span class="block text-[10px] text-red-500 dark:text-red-400 font-bold uppercase mt-0.5">Kedaluwarsa</span>
                                                @endif
                                            </span>
                                        @else
                                            <span class="text-slate-400 dark:text-slate-500">Seumur Hidup / -</span>
                                        @endif
                                    </td>
                                    <td class="px-5 py-4">
                                        @php
                                            $statusBadge = match($item->status) {
                                                'ada' => 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800',
                                                'revisi' => 'bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800',
                                                default => 'bg-rose-50 dark:bg-red-900/30 text-rose-700 dark:text-red-400 border-rose-200 dark:border-red-800'
                                            };
                                    ...
                                    <td class="px-5 py-4 text-xs text-slate-500 dark:text-slate-400 max-w-[200px] truncate" title="{{ $item->notes }}">
                                        {{ $item->notes ?: '-' }}
                                    </td>
                                    <td class="px-5 py-4">
                                        <div class="flex justify-end gap-1.5">
                                            @if ($item->file_path)
                                                <a href="{{ Storage::url($item->file_path) }}" target="_blank" class="rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-2 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 transition hover:bg-slate-50 dark:hover:bg-slate-600">
                                                    Lihat
                                                </a>
                                                <a href="{{ route('companies.workspace.documents.download', [$company, $item]) }}" class="rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-2 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 transition hover:bg-slate-50 dark:hover:bg-slate-600">
                                                    Unduh
                                                </a>
                                            @endif
                                            <a href="{{ route('companies.workspace.documents.edit', [$company, $item]) }}" class="rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-2.5 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 transition hover:bg-slate-50 dark:hover:bg-slate-600">
                                                Edit
                                            </a>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-5 py-12 text-center text-slate-500 dark:text-slate-400 font-medium">

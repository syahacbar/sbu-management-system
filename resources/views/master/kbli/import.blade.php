<x-layouts.admin title="Import Massal KBLI">
    <div class="space-y-5">
        <a href="{{ route('master.kbli.index') }}" class="inline-flex items-center gap-1.5 text-sm font-semibold text-slate-600 hover:text-slate-950 transition">
            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
            </svg>
            Kembali ke Daftar KBLI
        </a>
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            <section class="lg:col-span-2 rounded-lg border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 p-5">
                    ...
                    <p class="mt-1 text-sm text-slate-500">Unggah file format Excel (.xlsx) atau CSV yang berisi data KBLI.</p>
                </div>

                <form method="POST" action="{{ route('master.kbli.import') }}" enctype="multipart/form-data" class="p-5 space-y-5">
                    @csrf

                    <div>
                        <label for="file" class="block text-sm font-semibold text-slate-700">Pilih Berkas Berisi Data KBLI</label>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-200 pt-5">
                        <a href="{{ route('master.kbli.index') }}" class="rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                            Batal
                        </a>
            ...
            <aside class="space-y-6">
                <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <a href="{{ route('master.kbli.download-template') }}" class="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">
                ...
                <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm space-y-4">
                    ...
                    <ul class="text-xs text-slate-600 space-y-3 list-decimal pl-4">
                        <li>
                            ...
                            <br>Harus unik di database. Contoh: <code class="bg-slate-100 px-1 rounded">41011</code>. Jika kode sudah ada, data lama akan diperbarui.
                        </li>
                        <li>
                            <strong class="text-slate-900">Kolom 2: Nama KBLI (Wajib)</strong>
                            <br>Nama deskriptif bidang KBLI. Contoh: <code class="bg-slate-100 px-1 rounded">Konstruksi Gedung Hunian</code>.
                        </li>
                        <li>
                        ...
                        <li>
                            ...
                            <br>Penjelasan singkat atau catatan tambahan mengenai KBLI.
                        </li>
                    </ul>
                </section>
            </aside>

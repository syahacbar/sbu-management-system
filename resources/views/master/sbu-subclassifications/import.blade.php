<x-layouts.admin title="Import Massal Subklasifikasi SBU">
    <div class="space-y-5">
        <a href="{{ route('master.subclassifications.index') }}" class="inline-flex items-center gap-1.5 text-sm font-semibold text-slate-600 hover:text-slate-950 transition dark:text-slate-400 dark:hover:text-white">
            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
            </svg>
            Kembali ke Daftar Subklasifikasi
        </a>
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            <section class="lg:col-span-2 rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
                <div class="border-b border-slate-200 p-5 dark:border-slate-700">
                    ...
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Unggah file format Excel (.xlsx) atau CSV yang berisi data Subklasifikasi SBU beserta relasi kodenya.</p>
                </div>

                <form method="POST" action="{{ route('master.subclassifications.import') }}" enctype="multipart/form-data" class="p-5 space-y-5">
                    @csrf

                    <div>
                        <label for="file" class="block text-sm font-semibold text-slate-700 dark:text-slate-300">Pilih Berkas Berisi Data Subklasifikasi</label>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-200 pt-5 dark:border-slate-700">
                        <a href="{{ route('master.subclassifications.index') }}" class="rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600">
                            Batal
                        </a>
            ...
            <aside class="space-y-6">
                <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    ...
                    <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">Unduh berkas template di bawah ini untuk melihat contoh format pengisian Subklasifikasi SBU yang didukung oleh sistem.</p>
                    
                    <a href="{{ route('master.subclassifications.download-template') }}" class="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600">
                ...
                <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm space-y-4 dark:border-slate-700 dark:bg-slate-800">
                    ...
                    <ul class="text-xs text-slate-600 space-y-3 list-decimal pl-4 dark:text-slate-400">
                        <li>
                            <strong class="text-slate-900 dark:text-slate-200">Kolom 1: Kode Klasifikasi (Wajib)</strong>
                            <br>Harus berupa Kode Klasifikasi SBU terdaftar. Contoh: <code class="bg-slate-100 px-1 rounded dark:bg-slate-700">BG</code> (Bangunan Gedung). Jika kode klasifikasi tidak ditemukan, baris akan ditolak.
                        </li>
                        <li>
                            <strong class="text-slate-900 dark:text-slate-200">Kolom 2: Kode Subklasifikasi (Wajib)</strong>
                            <br>Harus unik di database. Contoh: <code class="bg-slate-100 px-1 rounded dark:bg-slate-700">BG001</code>. Jika kode sudah ada, data lama akan diperbarui.
                        </li>
                        <li>
                            <strong class="text-slate-900 dark:text-slate-200">Kolom 3: Nama Subklasifikasi (Wajib)</strong>
                            <br>Nama deskriptif Subklasifikasi. Contoh: <code class="bg-slate-100 px-1 rounded dark:bg-slate-700">Konstruksi Gedung Hunian</code>.
                        </li>
                        <li>
                            <strong class="text-slate-900 dark:text-slate-200">Kolom 4: Status (Opsional)</strong>
                        ...
                        <li>
                            <strong class="text-slate-900 dark:text-slate-200">Kolom 5: Urutan (Opsional)</strong>
                        ...
                        <li>
                            <strong class="text-slate-900 dark:text-slate-200">Kolom 6: Keterangan (Opsional)</strong>
                            <br>Penjelasan singkat atau catatan tambahan mengenai Subklasifikasi SBU.
                        </li>
                    </ul>
                </section>
            </aside>

    ...
    <div class="space-y-5">
        ...
        <section class="rounded-lg border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800">
            <div class="flex flex-col gap-4 border-b border-slate-200 p-5 lg:flex-row lg:items-center lg:justify-between dark:border-slate-700">
                <div>
                    <h3 class="text-lg font-semibold text-slate-950 dark:text-white">Master Subklasifikasi SBU</h3>
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Referensi global subklasifikasi SBU berdasarkan klasifikasi.</p>
                </div>

                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('master.subclassifications.import-form') }}" class="inline-flex items-center justify-center rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600">
                    ...
                    <a href="{{ route('master.subclassifications.create') }}" class="inline-flex items-center justify-center rounded-md bg-emerald-700 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-800 dark:bg-emerald-600 dark:hover:bg-emerald-700">
            ...
            <div class="border-b border-slate-200 p-5 dark:border-slate-700">
                <form method="GET" action="{{ route('master.subclassifications.index') }}" class="grid gap-3 xl:grid-cols-[1fr_220px_180px_auto]">
                    ...
                    <select name="classification_id" class="min-h-10 rounded-md border border-slate-300 px-3 py-2 text-sm outline-none transition focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-700">
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white dark:divide-slate-700 dark:bg-slate-800">
                        @forelse ($subclassifications as $subclassification)
                            <tr>
                                <td class="px-5 py-4">
                                    <p class="font-semibold text-slate-950 dark:text-white">{{ $subclassification->classification?->code }}</p>
                                    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ $subclassification->classification?->name }}</p>
                                </td>
                                <td class="px-5 py-4 font-semibold text-slate-950 dark:text-white">{{ $subclassification->code }}</td>
                                <td class="px-5 py-4">
                                    <p class="text-slate-800 dark:text-slate-200">{{ $subclassification->name }}</p>
                                    <p class="mt-1 max-w-md text-xs text-slate-500 dark:text-slate-400">{{ $subclassification->description ?: '-' }}</p>
                                </td>
                                <td class="px-5 py-4">
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('master.subclassifications.show', $subclassification) }}" class="rounded-md border border-slate-300 px-3 py-1.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700">
                                            Detail
                                        </a>
                                        <a href="{{ route('master.subclassifications.edit', $subclassification) }}" class="rounded-md border border-slate-300 px-3 py-1.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700">
                                            Edit
                                        </a>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-5 py-10 text-center text-slate-500 dark:text-slate-400">Belum ada data Subklasifikasi SBU.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>

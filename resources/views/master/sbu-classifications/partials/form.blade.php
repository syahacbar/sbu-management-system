<form method="POST" action="{{ $action }}" class="space-y-5 p-5">
    ...
    <div class="grid gap-5 md:grid-cols-2">
        <div>
            <label for="code" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Kode Klasifikasi</label>
            <input
                id="code"
                name="code"
                type="text"
        ...
        <div>
            ...
            <input
                ...
                type="number"
                min="0"
                value="{{ old('sort_order', $classification?->sort_order ?? 0) }}"
                required
                class="mt-2 block w-full rounded-md border border-slate-300 px-3 py-2 text-sm outline-none transition focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
            >
            @error('sort_order')
    ...
    <div>
        <label for="name" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Nama Klasifikasi</label>
        <input
            id="name"
            name="name"
            type="text"
    ...
    <div>
        ...
        <textarea
            ...
            rows="4"
            class="mt-2 block w-full rounded-md border border-slate-300 px-3 py-2 text-sm outline-none transition focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
        >{{ old('description', $classification?->description) }}</textarea>
        @error('description')
            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
        @enderror
    </div>
        ...
        <div class="mt-2 flex gap-4">
            <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                <input
                    ...
                    name="is_active"
                    value="1"
                    @checked((string) old('is_active', $classification?->is_active ?? true) === '1')
            ...
            <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                <input
                    ...
                    name="is_active"
                    value="0"
                    @checked((string) old('is_active', $classification?->is_active ?? true) === '0')
                    class="border-slate-300 text-emerald-700 focus:ring-emerald-600"
                >
                Nonaktif
            </label>

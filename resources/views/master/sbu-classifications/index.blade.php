<x-layouts.admin title="Klasifikasi SBU">
    <div class="space-y-5">
        @if (session('status'))
        ...
        <section class="rounded-lg border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800">
            <div class="flex flex-col gap-4 border-b border-slate-200 p-5 lg:flex-row lg:items-center lg:justify-between dark:border-slate-700">
                <div>
                    <h3 class="text-lg font-semibold text-slate-950 dark:text-white">Master Klasifikasi SBU</h3>
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Referensi global klasifikasi SBU untuk seluruh workspace perusahaan.</p>
                </div>

                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('master.classifications.import-form') }}" class="inline-flex items-center justify-center rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600">
                    ...
                    <a href="{{ route('master.classifications.create') }}" class="inline-flex items-center justify-center rounded-md bg-emerald-700 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-800 dark:bg-emerald-600 dark:hover:bg-emerald-700">
                        Tambah Klasifikasi
                    </a>
                </div>
            </div>
            ...
            <div class="border-b border-slate-200 p-5 dark:border-slate-700">
                <form method="GET" action="{{ route('master.classifications.index') }}" class="grid gap-3 lg:grid-cols-[1fr_180px_auto]">
                    <input
                        type="search"
                        name="search"
                        value="{{ $search }}"
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-700">
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white dark:divide-slate-700 dark:bg-slate-800">
                        @forelse ($classifications as $classification)
                            <tr>
                                <td class="px-5 py-4 text-slate-600 dark:text-slate-400">{{ $classification->sort_order }}</td>
                                <td class="px-5 py-4 font-semibold text-slate-950 dark:text-white">{{ $classification->code }}</td>
                                <td class="px-5 py-4 text-slate-800 dark:text-slate-200">{{ $classification->name }}</td>
                                <td class="px-5 py-4">
                                ...
                                <td class="max-w-md px-5 py-4 text-slate-600 dark:text-slate-400">{{ $classification->description ?: '-' }}</td>
                                <td class="px-5 py-4">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('master.classifications.show', $classification) }}" class="rounded-md border border-slate-300 px-3 py-1.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700">
                                            Detail
                                        </a>
                                        <a href="{{ route('master.classifications.edit', $classification) }}" class="rounded-md border border-slate-300 px-3 py-1.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700">
                                            Edit
                                        </a>
                                        <form method="POST" action="{{ route('master.classifications.destroy', $classification) }}" onsubmit="return confirm('Hapus data Klasifikasi SBU ini?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-5 py-10 text-center text-slate-500 dark:text-slate-400">Belum ada data Klasifikasi SBU.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>

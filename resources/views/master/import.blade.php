<x-layouts.admin title="Import Massal {{ $resource['title'] }}">
    <div class="space-y-5">
        <a href="{{ route($resource['route'].'.index') }}" class="inline-flex items-center gap-1.5 text-sm font-semibold text-slate-600 hover:text-slate-950 transition">
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            <section class="lg:col-span-2 rounded-lg border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 p-5">
                    ...
                    <p class="mt-1 text-sm text-slate-500">Unggah file format Excel (.xlsx) atau CSV yang berisi data {{ $resource['title'] }}.</p>
                </div>
                ...
                <form method="POST" action="{{ route($resource['route'].'.import') }}" enctype="multipart/form-data" class="p-5 space-y-5">
                    ...
                    <div>
                        <label for="file" class="block text-sm font-semibold text-slate-700">Pilih Berkas Berisi Data {{ $resource['title'] }}</label>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-200 pt-5">
                        <a href="{{ route($resource['route'].'.index') }}" class="rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                            Batal
                        </a>
            ...
            <aside class="space-y-6">
                <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <p class="mt-2 text-sm text-slate-600">Unduh berkas template di bawah ini untuk melihat contoh format pengisian yang didukung oleh sistem.</p>
                    
                    <a href="{{ route($resource['route'].'.download-template') }}" class="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">
                ...
                <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm space-y-4">
                    ...
                    <ul class="text-xs text-slate-600 space-y-3 list-decimal pl-4">
                        <li>
                            <strong class="text-slate-900">Kolom 1: Kode {{ $resource['title'] }} (Wajib)</strong>
                            <br>Harus unik di database. Jika kode sudah ada, data lama akan diperbarui.
                        </li>
                        <li>
                            <strong class="text-slate-900">Kolom 2: Nama {{ $resource['title'] }} (Wajib)</strong>

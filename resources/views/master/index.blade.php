    ...
    <div class="space-y-5">
        ...
        <section class="rounded-lg border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800">
            <div class="flex flex-col gap-4 border-b border-slate-200 p-5 lg:flex-row lg:items-center lg:justify-between dark:border-slate-700">
                <div>
                    <h3 class="text-lg font-semibold text-slate-950 dark:text-white">Master {{ $resource['title'] }}</h3>
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Kelola referensi global yang berdiri sendiri dari data perusahaan.</p>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    @if (in_array($resource['key'], ['science-fields', 'bg-equipment', 'bs-equipment']))
                        <a
                            ...
                            class="inline-flex items-center justify-center rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600"
                        >
                    ...
                    <a
                        ...
                        class="inline-flex items-center justify-center rounded-md bg-emerald-700 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-800 dark:bg-emerald-600 dark:hover:bg-emerald-700"
                    >
                        Tambah Data
                    </a>
                </div>
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white dark:divide-slate-700 dark:bg-slate-800">
                        @forelse ($items as $item)
                            <tr>
                                ...
                                <td class="px-5 py-4 font-semibold text-slate-950 dark:text-white">{{ $item->name }}</td>
                                <td class="px-5 py-4">
                                ...
                                <td class="max-w-md px-5 py-4 text-slate-600 dark:text-slate-400">{{ $item->description ?: '-' }}</td>
                                <td class="px-5 py-4">
                                    <div class="flex justify-end gap-2">
                                        <a
                                            href="{{ route($resource['route'].'.edit', $item) }}"
                                            class="rounded-md border border-slate-300 px-3 py-1.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700"
                                        >
                                            Edit
                                        </a>
